 ...
 */
  ...
  // ==========================================
  // H. Kontak Administrator Cabang untuk Lupa PIN
  // ==========================================
  public function kontak_admin_cabang()
  {
    header('Access-Control-Allow-Origin: *');
    header('Content-Type: application/json; charset=UTF-8');
    ...
    if ($auth->level !== 'Nasabah') {
      $this->api_response([
        'status'  => false,
        'message' => 'Kontak Administrator cabang hanya tersedia untuk Nasabah.'
      ], 403);
      return;
    }

    /*
     * Cabang selalu berasal dari Bearer token. Aplikasi tidak dapat
     * mengirim cabang_id lain untuk memperoleh kontak cabang berbeda.
     */
    $administrator = $this->db
      ->select('id, nama, telp')
      ->where('cabang_id', (int) $auth->cabang_id)
      ->where('level', 'Administrator')
      ->where('login', 'Ya')
      ->where("TRIM(telp) <> ''", null, false)
      ->order_by('id', 'ASC')
      ->get('tb_user')
      ->result();
    ...
    if (!$admin_terpilih) {
      $this->api_response([
        'status'  => false,
        'message' => 'Nomor WhatsApp Administrator cabang belum tersedia. Silakan hubungi kantor cabang Anda.',
        'data'    => [
          'cabang_id'   => (int) $auth->cabang_id,
          'kode_cabang' => $auth->kode_cabang,
          'nama_cabang' => $auth->nama_cabang
    ...
    $this->api_response([
      'status'  => true,
      'message' => 'Kontak Administrator cabang ditemukan.',
      'data'    => [
        'nama_admin'     => $admin_terpilih->nama,
