...
class Api extends CI_Controller
{
  ...
  public function login()
  {
    ...
    if ($_SERVER['REQUEST_METHOD'] !== 'POST' || empty($request)) {
      echo json_encode(['status' => false, 'message' => 'Permintaan tidak valid.']); return;
    }
    ...
    if (empty($username) || empty($password)) {
      echo json_encode(['status' => false, 'message' => 'Username dan Password harus diisi.']); return;
    }
    ...
    if ($user) {
      ...
      echo json_encode(['status' => false, 'message' => 'Username tidak terdaftar!']);
    }
  }
...
// ==========================================
  ...
  public function simpan_transaksi()
  {
    ...
    if (empty($data['idNasabah']) || empty($data['nominal']) || empty($data['jenis'])) {
      echo json_encode(['status' => false, 'message' => 'Data tidak lengkap!']); return;
    }
  ...
  public function update_profil()
  {
    ...
    if (empty($id_user) || empty($nama) || empty($username)) {
      echo json_encode(['status' => false, 'message' => 'Nama dan Username wajib diisi!']); return;
    }

    $this->db->where('username', $username);
    $this->db->where('id !=', $id_user);
    if ($this->db->get('tb_user')->num_rows() > 0) {
      echo json_encode(['status' => false, 'message' => 'Username sudah digunakan orang lain.']); return;
    }
  ...
  public function get_pengaturan() {
    ...
    if ($data) {
        ...
        echo json_encode([
            'status' => true, 
            'data' => [
                'nama' => '', 'telp' => '', 'email' => '', 'alamat' => '', 'logo' => null
            ]
        ]);
    }
  }
  ...
  public function upload_banner() {
    ...
    $admin = $this->db->get('tb_user')->row();
    if (!$admin) { echo json_encode(['status' => false, 'message' => 'Akses ditolak! Khusus Admin.']); return; }
  ...
  public function hapus_banner() {
    ...
    $this->db->where_in('level', ['Administrator', 'Super Admin']);
    $admin = $this->db->get('tb_user')->row();
    if (!$admin) { echo json_encode(['status' => false, 'message' => 'Akses ditolak!']); return; }
...
// Endpoint Admin: Ambil Semua Daftar Toko
  public function admin_get_semua_toko()
  {
      ...
      if ($level !== 'Administrator' && $level !== 'Super Admin') {
          echo json_encode(['status' => false, 'message' => 'Akses ditolak. Khusus Admin.']); return;
      }
...
// ==========================================
  ...
  public function cek_stok_keranjang()
  {
    ...
    if (empty($items)) {
      echo json_encode(['status' => false, 'message' => 'Keranjang kosong.']); return;
    }
...
// FASE 1: Pembeli Checkout (Status: Menunggu Ongkir) - Tanpa potong saldo & tanpa PIN
  public function checkout_belanja()
  {
    ...
    if (empty($id_pembeli) || empty($id_toko) || empty($items)) {
      echo json_encode(['status' => false, 'message' => 'Data pesanan tidak lengkap.']); return;
    }
 ...
 // FASE 2: Penjual Input Ongkir (Status: Menunggu Pembayaran)
  public function input_ongkir_penjual()
  {
    ...
    if (empty($id_pesanan)) {
      echo json_encode(['status' => false, 'message' => 'ID Pesanan tidak valid.']); return;
    }
...
// FASE 3: Pembeli Membayar Pesanan (Status: Diproses) - Menggunakan PIN & Potong Saldo
  public function bayar_pesanan()
  {
    ...
    if (empty($id_pesanan) || empty($id_pembeli) || empty($pin)) {
      echo json_encode(['status' => false, 'message' => 'Data tidak lengkap atau PIN kosong.']); return;
    }
    ...
    $user = $this->db->get('tb_user')->row();
    if (!$user || $user->pin !== $pin) {
      echo json_encode(['status' => false, 'message' => 'PIN transaksi salah!']); return;
    }
    ...
    if (!$pesanan) {
      echo json_encode(['status' => false, 'message' => 'Pesanan tidak ditemukan atau sudah dibayar.']); return;
    }
    ...
    if ($saldo_aktif < $grand_total) {
      echo json_encode(['status' => false, 'message' => 'Saldo tabungan tidak mencukupi. Total belanja + ongkir adalah Rp ' . number_format($grand_total, 0, ',', '.')]); return;
    }
...
// 8. Endpoint Update Status Pesanan (Oleh Penjual)
 ...
 // 9. Endpoint Selesaikan Pesanan & CAIRKAN DANA KE PENJUAL (Oleh Pembeli)
  public function terima_pesanan()
  {
    ...
    if (!$pesanan) {
      echo json_encode(['status' => false, 'message' => 'Pesanan tidak ditemukan.']); return;
    }

    if ($pesanan->status_pesanan === 'Selesai') {
      echo json_encode(['status' => false, 'message' => 'Pesanan ini sudah diselesaikan sebelumnya.']); return;
    }
  ...
  public function batalkan_pesanan()
  {
    ...
    if (!$pesanan || ($pesanan['status_pesanan'] !== 'Menunggu Ongkir' && $pesanan['status_pesanan'] !== 'Menunggu Pembayaran')) {
      echo json_encode(['status' => false, 'message' => 'Pesanan sudah dibayar/diproses dan tidak bisa dibatalkan sendiri.']); return;
    }
...
// 11. Endpoint Hapus Produk (Soft Delete)
  public function hapus_produk()
  {
    ...
    if (empty($id_produk)) {
      echo json_encode(['status' => false, 'message' => 'ID Produk tidak valid.']); return;
    }
...
// 13. Endpoint Admin: Batalkan Paksa Pesanan & Refund
  public function admin_batalkan_pesanan()
  {
    ...
    if (!in_array($level, ['Administrator', 'Super Admin']) || empty($id_pesanan)) {
      echo json_encode(['status' => false, 'message' => 'Akses ditolak atau data tidak valid.']); return;
    }
    ...
    if (!$pesanan || $pesanan['status_pesanan'] === 'Selesai' || $pesanan['status_pesanan'] === 'Dibatalkan') {
      echo json_encode(['status' => false, 'message' => 'Pesanan sudah selesai atau sudah dibatalkan sebelumnya.']); return;
    }
...
// 12. Endpoint Simpan Ulasan Pembeli
  public function simpan_ulasan()
  {
    ...
    if (empty($id_pesanan) || empty($id_pembeli)) {
      echo json_encode(['status' => false, 'message' => 'Data tidak valid.']); return;
    }
...
// 15. Endpoint Cek Status Wishlist di Detail Produk
  public function cek_wishlist()
  {
    ...
    if (empty($id_pembeli) || empty($id_produk)) {
      echo json_encode(['status' => false, 'is_wishlisted' => false]); return;
    }
...
// 17. Endpoint Ambil Daftar Wishlist Pembeli
  ...
  public function count_wishlist()
  {
    ...
    if (empty($id_user)) {
      echo json_encode(['status' => false, 'count' => 0]); return;
    }
...
// 22. Endpoint Hapus Semua Notifikasi User
  public function clear_notifikasi()
  {
    ...
    if (empty($id_user)) {
      echo json_encode(['status' => false, 'message' => 'ID User tidak valid.']); return;
    }
 ...
 // 2. Upload Banner Marketplace (Bebas Cache + Push Notif)
  ...
  public function hapus_banner_market() {
      ...
      if ($banner) {
          ...
          if (file_exists($file_path)) { unlink($file_path); } // Hapus file fisik
...
if (!$admin) {
    echo json_encode(['status' => false, 'message' => 'Akses ditolak! Khusus Super Admin.']); return;
}

      if (empty($_FILES['file_apk']['name'])) {
          echo json_encode(['status' => false, 'message' => 'File APK belum dipilih.']); return;
      }
      ...
      if (strtolower($file_ext) !== 'apk') {
          echo json_encode(['status' => false, 'message' => 'Format file tidak diizinkan. Harus berupa .apk!']); return;
      }
 ...
 // ==========================================
  ...
  public function get_semua_saldo_eksternal()
  {
      ...
      if (empty($request) || ($request['api_key'] ?? '') !== $api_key_valid) {
          echo json_encode(['status' => false, 'message' => 'API Key tidak valid!']); return;
      }
